...
import com.ibm.icu.math.IcuBigDecimal;
 ...
 * com.ibm.icu.math.IcuBigDecimal} documentation for a description of the modes.  Rounding
 * increments specified in patterns use the default mode, {@link
 * com.ibm.icu.math.IcuBigDecimal#ROUND_HALF_EVEN}.
 *
...
public class DecimalFormat extends NumberFormat {
    ...
    private static double round(double number, double roundingInc, double roundingIncReciprocal,
        ...
        default:
            ...
            switch (mode) {
            case IcuBigDecimal.ROUND_HALF_EVEN:
                ...
                break;
            case IcuBigDecimal.ROUND_HALF_DOWN:
                div = ((floordiff <= ceildiff + epsilon) ? floor : ceil);
                break;
            case IcuBigDecimal.ROUND_HALF_UP:
    ...
    private StringBuffer format(long number, StringBuffer result, FieldPosition fieldPosition,
        ...
        // If we are to do rounding, we need to move into the IcuBigDecimal
        // domain in order to do divide/multiply correctly.
        if (roundingIncrementICU != null) {
            return format(IcuBigDecimal.valueOf(number), result, fieldPosition);
        }
    ...
    private StringBuffer format(BigInteger number, StringBuffer result, FieldPosition fieldPosition,
        ...
        // domain in order to do divide/multiply correctly.
        if (roundingIncrementICU != null) {
            return format(new IcuBigDecimal(number), result, fieldPosition);
        }
    ...
    /**
     * Formats a IcuBigDecimal number.
     *
     * @stable ICU 2.0
     */
     ...
     * @stable ICU 2.0
     */
    public StringBuffer format(IcuBigDecimal number, StringBuffer result,
                               FieldPosition fieldPosition) {
         // This method is just a copy of the corresponding java.math.IcuBigDecimal method
         ...
         // or even better, limit our support to just one flavor of IcuBigDecimal.
        if (multiplier != 1) {
            number = number.multiply(IcuBigDecimal.valueOf(multiplier), mathContext);
        }
     ...
     * values which cannot be stored in a <code>IcuBigDecimal</code>. These are
     ...
     * <code>BigInteger</code>, or <code>IcuBigDecimal</code> values, in that order of
     * preference. If {@link #isParseBigDecimal()} is true, all other values are returned
     * as <code>IcuBigDecimal</code> valuse. If the parse fails, null is returned.
     *
    ...
    private Object parse(String text, ParsePosition parsePosition, Currency[] currency) {
        ...
        // Handle -0.0
            ...
            // Handle non-integral values or the case where parseBigDecimal is set
                ...
                IcuBigDecimal big = digitList.getBigDecimalICU(status[STATUS_POSITIVE]);
                n = big;
                if (mult != 1) {
                    n = big.divide(IcuBigDecimal.valueOf(mult), mathContext);
                }
            }
        }
...
//            0xFF07, 0xFF07).freeze();
     ...
     */
//#if defined(ECLIPSE)
//##    public IcuBigDecimal getRoundingIncrement() {
...
//#endif
     ...
     * <code>IcuBigDecimal(0.0)</code> to disable rounding.
    ...
    public void setRoundingIncrement(java.math.BigDecimal newValue) {
        if (newValue == null) {
            setRoundingIncrement((IcuBigDecimal) null);
        } else {
            setRoundingIncrement(new IcuBigDecimal(newValue.toString()));
        }
    }
     ...
     * @stable ICU 3.6
     */
    public void setRoundingIncrement(IcuBigDecimal newValue) {
        int i = newValue == null ? 0 : newValue.compareTo(IcuBigDecimal.ZERO);
        if (i < 0) {
    ...
    public void setRoundingIncrement(double newValue) {
        ...
        if (newValue == 0.0d) {
            ...
            setInternalRoundingIncrement(new IcuBigDecimal(newValue));
        }
    }
     ...
     *
     * @return A rounding mode, between <code>IcuBigDecimal.ROUND_UP</code> and
     * <code>IcuBigDecimal.ROUND_UNNECESSARY</code>.
     * @see #setRoundingIncrement
     * @see #getRoundingIncrement
     * @see #setRoundingMode
     ...
     * than zero.
     *
     * @param roundingMode A rounding mode, between <code>IcuBigDecimal.ROUND_UP</code> and
     * <code>IcuBigDecimal.ROUND_UNNECESSARY</code>.
    ...
    public void setRoundingMode(int roundingMode) {
        if (roundingMode < IcuBigDecimal.ROUND_UP || roundingMode > IcuBigDecimal.ROUND_UNNECESSARY) {
    ...
    private void applyPatternWithoutExpandAffix(String pattern, boolean localized) {
        ...
        for (int part = 0; part < 2 && pos < pattern.length(); ++part) {
            ...
            if (part == 0) {
                ...
                if (incrementVal != 0) {
                    // IcuBigDecimal scale cannot be negative (even though this makes perfect
                    // sense), so we need to handle this.
                    int scale = incrementPos - effectiveDecimalPos;
                    roundingIncrementICU = IcuBigDecimal.valueOf(incrementVal, scale > 0 ? scale : 0);
                    if (scale < 0) {
                        roundingIncrementICU = roundingIncrementICU.movePointRight(-scale);
                    }
                    setRoundingDouble();
                    roundingMode = IcuBigDecimal.ROUND_HALF_EVEN;
                } else {
                    setRoundingIncrement((IcuBigDecimal) null);
                }
    ...
    /**
     * Sets whether {@link #parse(String, ParsePosition)} returns IcuBigDecimal. The
     * default value is false.
     *
     * @param value true if {@link #parse(String, ParsePosition)}
     * returns IcuBigDecimal.
     * @stable ICU 3.6
     */
    ...
    /**
     * Returns whether {@link #parse(String, ParsePosition)} returns IcuBigDecimal.
     *
     * @return true if {@link #parse(String, ParsePosition)} returns IcuBigDecimal.
     * @stable ICU 3.6
     */
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        if (roundingIncrement != null) {
            setInternalRoundingIncrement(new IcuBigDecimal(roundingIncrement.toString()));
            setRoundingDouble();
        }
    }

    private void setInternalRoundingIncrement(IcuBigDecimal value) {
     ...
     * @since AlphaWorks NumberFormat
     */
    private transient IcuBigDecimal roundingIncrementICU = null;
     ...
     * <code>IcuBigDecimal</code> rounding mode values. Default value
     * <code>IcuBigDecimal.ROUND_HALF_EVEN</code>.
     *
     * @serial
     * @since AlphaWorks NumberFormat
     */
    private int roundingMode = IcuBigDecimal.ROUND_HALF_EVEN;

    /**
     * Operations on <code>IcuBigDecimal</code> numbers are controlled by a {@link
     ...
     * for the original IcuBigDecimal class in Java 1.1 and Java 1.2
     */
    ...
    /**
     * True if {@link #parse(String, ParsePosition)} to return IcuBigDecimal rather than
     * Long, Double or IcuBigDecimal except special values. This property is introduced for
