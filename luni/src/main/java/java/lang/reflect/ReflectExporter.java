...
class ReflectExporter implements ReflectAccessor
{
	...
	public Method[] mergePublicMethods(Method[] declared, Method[] superPublic, Method[][] intf, int estimate)
	{
		...
		{
			nextSuper:
			for(Method m : superPublic)
			{
				for(int i = 0; i < size; i++)
				{
					if(m.getName() == store[i].getName() && m.getSignature() == store[i].getSignature())
					{
						continue nextSuper;
					}
				}
	...
	public void checkMemberAccess(Class callerClass, Class declaringClass, Class runtimeClass, int memberModifiers) throws IllegalAccessException
	{
		if(!allowAccess(callerClass, declaringClass, runtimeClass, memberModifiers))
		{
			throw new IllegalAccessException("A member of the \"" + declaringClass + "\" with \"" + Modifier.toString(memberModifiers) + "\" modifiers can not be accessed from the \"" + callerClass + "\"");
		}
	}
	...
	private boolean allowAccess(Class<?> callerClass, Class<?> declaringClass, Class<?> runtimeClass, int memberModifiers)
	{
		...
		if(Modifier.isPublic(memberModifiers))
		{
			...
			if(runtimeClass != declaringClass)
			{
				do
				{
					if(allowClassAccess(runtimeClass, callerClass) || hasSameTopLevelClass(runtimeClass, callerClass))
					{
						return true;
					}
				}
		...
		if(hasSamePackage(declaringClass, callerClass) && allowClassAccess(declaringClass, callerClass))
		{
			return true;
		}
